...
const Sidebar = () => {
  ...
  function getRooms() {
    fetch("https://chat-app-serv.herokuapp.com/rooms").then((res) => res.json()).then((data) => setRooms(data));
  }
        ...
        <div className="rooms">
            {members.map((member) => (
                <button onClick={() => handlePrivateMemberMsg(member)} className={privateMemberMsg?._id === member?._id ? "activeRoom" : ""} key={member._id} disabled={member._id === user._id}>
                  ...
                  {member.name}
                  {member.status === "Online" ? <SignalCellular4BarRoundedIcon /> : <ModeNightOutlinedIcon />}
